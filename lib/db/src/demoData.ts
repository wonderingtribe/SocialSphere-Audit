...
// Demo dataset. Every record is explicitly marked DEMO so clients can always
// tell simulated preview data apart from REAL platform data (product guardrail).
// Real platform events will replace these once official OAuth integrations exist.
...
export function advanceDemoLead(lead: Lead): Lead | null {
  if (lead.mode !== "DEMO") return null;

  const currentIndex = statusOrder.indexOf(lead.status);
  const nextIndex = currentIndex + 1;
  if (nextIndex >= statusOrder.length) return null;

  return { ...lead, status: statusOrder[nextIndex] };
}